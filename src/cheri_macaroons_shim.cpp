...
void print_cheri_macaroons_shim_info(std::string function)
{
    std::cout << display_marker << std::endl;
    std::cout << "in cheri_macaroons_shim" << std::endl;
...
{
    print_cheri_macaroons_shim_info(std::string(__FUNCTION__));
 ...
 *
 * If shim == NONE, then this simply calls libmodbus:modbus_process_request
 *
 * If shim == CHERI_SHIM, this will call the function in cheri_shim, which
 * will call libmodbus:modbus_process_request
 *
 * If shim == MACAROONS_SHIM, this will call the function in macaroon_shim, which
 * will call libmodbus:modbus_process_request
 *
 * (the important bit)
 * If shim == CHERI_MACAROON_SHIM, this will:
 * 1. call the function in cheri_shim to set permissions on mb_mapping
 * 2. the function in cheri_shim will call this function again with shim == CHERI_SHIM_X
 * 3. call the function in macaroon_shim, which
 * 4. the function in macaroon_shim will call libmodbus:modbus_process_request
 * */
